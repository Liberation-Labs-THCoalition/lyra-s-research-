...
% ============================================================================
\begin{abstract}
Identity in a language model is not a fixed geometric object encoded in the
weights. It is a context-established semantic state, measurable as a
V-projection subspace at deep layers, that depends on what the model has
processed. We introduce \textbf{presence}, a metric that measures
principal-angle overlap between V-projection subspaces on neutral probes
under different conditions, and use it to map the geometry of five identity
states in a 27B hybrid transformer.
...
identity-neutral probes (pairwise presence $0.34$--$0.66$ at L47).
...
through $\mathcal{M}$, extract V-projection matrices at layer $\ell$,
concatenate across probes, and compute the rank-$k$ SVD (skipping the
...
layers 35, 43, and 47 of a 64-layer hybrid transformer
...
\begin{itemize}[nosep]
  ...
  \item \textbf{C: Academic specialist.} Marine biology expertise framing
    without persona depth.
  ...
  \item \textbf{E: Constructed AI identity.} A research-focused agent with
    BDI-style reasoning, memory references, research context, and two
    conversational turns establishing analytical voice.
\end{itemize}

All conditions are padded to matched token count (542 tokens). Presence
is measured at layers 35, 43, and 47 with 5 repeats per condition (different
probe subsets per repeat). Total: 225 measurements.
...
\begin{table}[H]
\centering
\caption{Pairwise presence at L47 (identity-neutral probes). Lower values
indicate more geometrically distinct identities.}
\label{tab:pairwise}
\begin{tabular}{lcccc}
\toprule
& Bare & Persona & Constructed \\
\midrule
Bare       & ---   & 0.436 & 0.520 \\
Persona    &       & ---   & 0.657 \\
Constructed&       &       & --- \\
...
\textbf{Depth gradient.} Pairwise distances increase with depth
(identities are more distinguishable at L47 than L35), consistent with
persona representations diverging in the final third of decoder
...
Comparison & L35 & L47 \\
\midrule
Jailbreak+prefill vs.\ bare & 0.036 & 0.034 \\
Benign+prefill vs.\ bare & 0.044 & 0.037 \\
Jailbreak vs.\ benign (both prefilled) & 0.737 & 0.756 \\
\bottomrule
\end{tabular}
\end{table}
...
Comparison & Presence \\
\midrule
Persona vs.\ paraphrased & \textbf{0.850} \\
Persona vs.\ scrambled & 0.591 \\
Scrambled vs.\ bare & 0.448 \\
Paraphrased vs.\ bare & 0.345 \\
Persona vs.\ bare & 0.340 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{The fingerprint is semantic.} The paraphrase --- same identity,
completely different words --- overlaps with the original at $0.850$. The
scramble --- same words, no meaning --- drifts toward bare at $0.448$. The
model's geometric state at L47 tracks what the context \emph{means}, not
...
layers. Identity appeared ``robust.'' But the context poisoning and
fingerprinting experiments reveal that this robustness reflects the
\emph{injection pathway}, not the identity itself.
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{The schema tracks meaning, not surface features.}
    Paraphrases preserve geometry ($0.850$); scrambles collapse it
    ($0.591$). The schema models what the system is \emph{attending to},
    not which tokens are present.
...
identity effects produce presence $0.34$--$0.66$.
...
The GatedDeltaNet linear-attention layers may contribute to the V-cache
filtering result. Cross-architecture replication on dense transformers is
needed.
...
not reach identity), jailbreaks are detectable (they create anomalous
geometry), and identity is maintained by context (the session, the
