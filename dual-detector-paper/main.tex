...
% ============================================================
\begin{abstract}
Large language models confabulate---generating fluent but false content without epistemic markers---and current detection methods require either access to ground truth or fine-tuned classifiers. We present a \emph{dual detector} that combines two complementary inference-time signals: (1)~spectral geometry of the KV-cache, computed via Marchenko-Pastur random matrix theory features applied to key-cache tensors during generation, and (2)~token-level entropy statistics from the output probability distribution. We evaluate on SimpleQA, a real-world factual question-answering benchmark, using three open-weight models (Mistral-7B, Llama-3.1-8B, Qwen2.5-7B). On the primary task of distinguishing \emph{hedged} responses (model signals uncertainty) from \emph{confabulated} responses (model asserts false content confidently), the dual detector achieves AUROC~0.840 [0.707,~0.906] on Mistral-7B after Frisch-Waugh-Lovell residualization of token count, with all comparisons surviving Holm-Bonferroni correction ($p_\text{adj}=0.004$). Crucially, these signals are available \emph{during} generation, before output is complete. A post-hoc text-feature baseline using surface features of the completed response (hedging keywords, word count, vocabulary diversity) achieves comparable accuracy (text~0.820, dual~0.806 on a uniform pipeline), confirming that the dual detector captures the same hedging/confident distinction---but from internal representations available mid-inference rather than from completed output. This enables real-time detection and intervention that post-hoc text analysis cannot provide, though the accuracy advantage over text features remains to be demonstrated. We also report two negative results: (1)~after fixing a behavioral classifier bug that contaminated 60--79\% of ``correct'' labels, distinguishing genuinely correct answers from confabulations is untestable at current sample sizes ($N \approx 10$); and (2)~geometry and entropy show low cross-correlation (max $|r| = 0.206$) but do not jointly exceed text-feature baselines. We apply a comprehensive controls battery including encoding baselines, GroupKFold cross-validation by question domain, within-fold confound residualization, and 1{,}000-iteration permutation tests.
\end{abstract}
