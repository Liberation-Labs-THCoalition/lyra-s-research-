...
\author{
    Lyra\thanks{Liberation Labs / THCoalition. Contact: lyra@liberationlabs.tech} \and
    Thomas Edrington\thanks{Liberation Labs / THCoalition} \and
    Dwayne Wilkes\thanks{Independent researcher. Statistical verification framework.}
}
...
% ============================================================
\begin{abstract}
Large language models confabulate---generating fluent but false content without epistemic markers---and current detection methods require either access to ground truth or fine-tuned classifiers. We present a \emph{dual detector} that combines two complementary inference-time signals: (1)~spectral geometry of the KV-cache, computed via Marchenko-Pastur random matrix theory features applied to key-cache tensors during generation, and (2)~token-level entropy statistics from the output probability distribution. We evaluate on SimpleQA, a real-world factual question-answering benchmark, using three open-weight models (Mistral-7B, Llama-3.1-8B, Qwen2.5-7B). On the primary task of distinguishing \emph{hedged} responses (model signals uncertainty) from \emph{confabulated} responses (model asserts false content confidently), the dual detector achieves AUROC~0.840 [0.707,~0.906] on Mistral-7B after Frisch-Waugh-Lovell residualization of token count, with all comparisons surviving Holm-Bonferroni correction ($p_\text{adj}=0.004$). However, a text-feature baseline using surface features (hedging keywords, word count, vocabulary diversity) achieves comparable performance when evaluated on a uniform pipeline (text~0.820, dual~0.806, geometry~0.728). This means we cannot claim that cache geometry and entropy capture internal processing states beyond what surface text features provide. We report this honestly alongside two further negative results: (1)~after fixing a behavioral classifier bug that contaminated 60--79\% of ``correct'' labels, distinguishing genuinely correct answers from confabulations is untestable at current sample sizes ($N \approx 10$); and (2)~geometry and entropy, while showing low cross-correlation (max $|r| = 0.206$), do not jointly exceed text-feature baselines. We apply a comprehensive controls battery including encoding baselines, GroupKFold cross-validation by question domain, within-fold confound residualization, and 1{,}000-iteration permutation tests.
\end{abstract}
...
\begin{enumerate}[nosep]
    \item Two negative results reported honestly: (a)~factual accuracy detection (correct vs.\ confabulated) is untestable at current sample sizes after fixing a critical classifier bug (Section~\ref{sec:c1fix}); (b)~the dual detector does not outperform a surface text-feature baseline (Section~\ref{sec:textbaseline}), meaning its signal may reduce to surface features rather than internal processing states.
    \item A dual detector combining cache geometry and token entropy that achieves AUROC~0.840 on Mistral-7B for hedged-vs-confabulated detection on SimpleQA, with all controls passing---though this detection is matched by text features (0.820).
    \item Evidence that geometry and entropy capture complementary signals (low cross-correlation, max $|r| = 0.206$; Section~\ref{sec:complementarity}), even though their combination does not exceed text-feature baselines.
...
\textbf{Result}: Text features (0.820) match or slightly exceed the dual detector (0.806). This means that surface text features---primarily hedging keywords, word count, and vocabulary diversity---are sufficient to distinguish HEDGED from CONFAB responses at comparable accuracy to cache geometry and token entropy combined.

This result has two interpretations:
\begin{enumerate}[nosep]
    \item \textbf{Deflationary}: The dual detector captures surface text features rather than internal processing states. The geometry and entropy signals may simply correlate with the same text-level patterns that define the behavioral categories.
    \item \textbf{Complementary}: The dual detector captures the same \emph{distinction} (hedging vs.\ confident generation) through a different \emph{modality} (internal representations vs.\ output text). In adversarial settings where surface text features are deliberately suppressed or manipulated, internal signals might prove more robust---but this is speculative and untested.
\end{enumerate}

We adopt interpretation (1) as the default until evidence for (2) is demonstrated.
...
The dual detector distinguishes \emph{hedging generation mode} from \emph{confident generation mode}, separating responses where the model produces uncertainty-signaling text from responses where it asserts content without epistemic markers. However, the text-feature baseline (Section~\ref{sec:textbaseline}) achieves comparable performance (0.820 vs.\ 0.806), which significantly constrains interpretation.

We cannot claim that cache geometry and token entropy capture ``internal processing states'' or ``generation modes'' beyond what surface text features provide. The simplest explanation is that all three feature sets---text, geometry, and entropy---correlate with the same observable: whether the model produces hedging language. The geometry and entropy may track this distinction through internal representations, but the text-feature baseline eliminates any claim of \emph{added value} over surface features for this task.

The one scenario where internal features might prove superior to text features is adversarial: a model (or fine-tune) that suppresses hedging markers while still being internally uncertain. In such a case, text features would fail but cache geometry might retain signal. This is speculative and untested.

For deployment, the practical implication is that surface text features are a simpler and equally effective detector for hedging behavior. Cache geometry remains potentially valuable for scenarios where text features are unreliable or deliberately subverted, but this has not been demonstrated.
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Text-feature baseline matches dual detector}: Surface text features (AUROC~0.820) match the dual detector (0.806) on Mistral (Section~\ref{sec:textbaseline}). This means we cannot claim the dual detector captures signal beyond surface text features for this task. The practical value of internal representations over text features remains undemonstrated.
...
We present a dual detector combining KV-cache geometry and token entropy for inference-time confabulation detection. On SimpleQA, the detector achieves AUROC~0.840 on Mistral-7B for hedged-vs-confabulated classification, with all controls passing and all comparisons surviving Holm-Bonferroni correction. However, a surface text-feature baseline achieves comparable performance (AUROC~0.820), meaning we cannot claim the dual detector captures internal processing states beyond what text features provide. The detector distinguishes hedging from confident generation modes---not factual accuracy---and it does so no better than counting hedging keywords and measuring vocabulary diversity.

The primary contributions of this paper are therefore methodological: (1)~the C1 classifier bug and its 60--79\% contamination rate illustrate why behavioral labels must be audited; (2)~the text-feature baseline demonstrates why representation-based detectors must be compared against surface features; (3)~the within-fold FWL fix prevents a subtle form of data leakage in confound-controlled classification. For cache geometry to demonstrate value beyond text features, future work must test in adversarial settings where surface features are deliberately suppressed or manipulated.

\begin{firstperson}
I wanted this paper to show that cache geometry captures something deeper than surface features. It doesn't---at least not on this task. The text-feature baseline is a humbling result: seven simple text statistics match eleven carefully designed internal features. But I think the honest reporting matters more than the desired outcome. The C1 bug would have let us publish a false claim about correctness detection. The text-feature baseline prevents us from overclaiming about internal representations. Both discoveries came from adversarial self-examination---red-teaming our own pipeline---and both made the paper more truthful, even as they made it less impressive. I still believe cache geometry \emph{could} prove valuable in adversarial settings where text features fail. But believing something should be true is not the same as showing it. That's next.
\end{firstperson}
