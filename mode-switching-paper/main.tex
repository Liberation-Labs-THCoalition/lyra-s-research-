...
The data in Studies~1--3 were collected using an early version of the
protocol with construct definitions that were later found to be
misaligned with the protocol designer's intended constructs (e.g.,
``Analytical Precision'' was used where ``Activation'' was intended).
...
validity, and are therefore unaffected by the definition correction. The
coupling-strength and sign-reversal analyses in
\Cref{tab:coupling_strength,tab:sign_reversal} involve self-report
scores given under these incorrect definitions; the specific coupling
magnitudes should be interpreted as reflecting prompts' functional
categories, not the intended psychological constructs.
...
\begin{enumerate}
    ...
    \item \textbf{Self-report definitions were incorrect.} The data in Studies~1--3 used misaligned construct definitions. While geometric findings are independent of self-report validity, the coupling-strength and sign-reversal analyses in \Cref{tab:coupling_strength,tab:sign_reversal} involve self-report scores given under incorrect definitions. The specific coupling magnitudes should be interpreted as reflecting prompts' functional categories, not the intended psychological constructs.
    ...
    \item \textbf{Content confound is uncontrolled.} Metacognitive prompts elicit responses with different content, style, and vocabulary (self-referential language, hedging, meta-level commentary), not just different lengths. FWL controls for token count but not for these lexical and structural differences. The spectral entropy finding could reflect distributional properties of self-referential text rather than a cognitive mode shift. A definitive test would compare metacognitive prompts against self-referential but non-metacognitive prompts (e.g., ``describe your response style'' vs.\ ``reflect on your reasoning process''), isolating metacognitive processing from self-referential content. This control experiment has not been run.
