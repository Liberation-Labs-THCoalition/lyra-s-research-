...
cache-geometry techniques. We discuss limitations including small sample
size ($n=7$ confabulations on the distilled model), marginal
cache-vs-text added value, reliance on TEE hardware trust assumptions,
underpowered injection testing, and the gap between our
orthogonal-rotation simulation and KV-Cloak's full implementation
(in-progress).
...
degrades on the distilled model (0.786). Magnitude-based attacks
...
\item \textbf{Simulation, not implementation.} We simulate KV-Cloak as
per-head orthogonal rotation. The actual KV-Cloak mechanism
($S \cdot \hat{P} \cdot (K + A) \cdot M$) includes additive masking
and per-block permutation that our simulation omits. A test using the
full mechanism is in progress. Results represent the effect of the
rotation component specifically.
