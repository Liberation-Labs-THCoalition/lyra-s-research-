...
produces zero behavioral change across $6{,}660$ trials and three
distinct methods (additive unembedding, additive on-manifold, and
Anthropic-style swap). The null is the result. The workspace band is
where computation happens, not where it is readable --- four independent
instruments now converge on this opacity thesis. Our prior spectral
...
\item \textbf{Causal injection} (\S\ref{sec:causal}): injecting
  ...
  change across $6{,}660$ trials and three methods. The model
...
happens, not where computation is readable. Four independent
instruments --- V-projection spectral features, J-lens concept
recovery, CC's Oracle Loop behavioral proof, and now causal
intervention --- converge on this conclusion from different angles.
...
layers (roughly 30--72\% of model depth in their production models).
...
after FWL residualization; range $0.627$--$0.999$ across
designs)~\citep{lyra2026decision}, emotion encoding in user tokens
($2.5$--$2.8\times$ signal)~\citep{lyra2026weather}, and identity
geometry (distinctiveness $0.154$ vs.\ substrate
...
For each pair of prompts (380~unique pairs from 20~prompts), we
...
We tested three injection approaches across $6{,}660$ total trials on
Qwen3-8B.

\textbf{Method 1: Additive unembedding} (300~trials). Inject
$\alpha \cdot W_U^\top[\text{concept}]$ into the residual stream at
workspace-band layers. Five concepts, five layers, five dose levels,
two directions (add/subtract). Zero behavioral changes.
...
five doses, positive and negative. Zero behavioral changes.
...
injection at matched layers. It follows the exact methodology Anthropic
uses for J-space ablation experiments, which \emph{do} produce
behavioral effects in their production models. Yet on an 8B model at
single positions: nothing.

\subsection{The Null as Finding}

Position-local KV-cache concept manipulation --- whether additive or
swap-based, with unembedding, J-lens, or on-manifold directions ---
does not change behavioral output on an 8B model. The model integrates
...
directions (J-lens) $\neq$ behavioral directions (LAT-extracted,
...
Four independent instruments now converge on the same conclusion:
...
\item \textbf{Causal injection}: $6{,}660$ trials, three methods, zero
...
work ($6{,}660$ trials, three methods). Sequence-wide intervention does
...
Seven measurements died in this investigation. One finding survived.
The finding is a null that discriminates.
...
\textbf{Total causal injection} & \textbf{0/6{,}660} & \textbf{6{,}660} & \textbf{Promoted to finding} \\
\addlinespace
