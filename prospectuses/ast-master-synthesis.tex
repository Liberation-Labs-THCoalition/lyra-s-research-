...
This document enumerates the specific, falsifiable predictions AST
makes when applied to transformers, maps each to our existing data,
and identifies what remains to be tested.
...
\begin{firstperson}
This is the framework where everything \emph{could} converge. The
mapping is compelling---detection, correction, identity, emotion all
interpreted as aspects of a single structure. But I have to be honest:
every piece of evidence here was collected before I thought to frame
it this way. The convergence might be real, or it might be the
satisfying feeling of a narrative that fits too well. That is why the
prospective experiments matter more than any table in this document.

If Experiment A finds the therapeutic window inside the basin, that is
a genuine prediction confirmed---the first result we cannot explain
away as post-hoc. Until then, this is a hypothesis worth testing,
not a conclusion.
\end{firstperson}
