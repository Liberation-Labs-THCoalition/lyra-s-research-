...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Steering validated at proof-of-concept scale only.}
    ...
    on $N = 7$ confabulations with a single vector and single model.
    The 71\% correction rate has wide confidence intervals. A powered
    replication across models and vectors is needed before deployment
    claims.
...
injections on both architectures at all levels, including the weakest
strength of $\alpha = 0.01$---two orders of magnitude below the
operational strength used in our steering experiment. False positive rate is 0/36 across both models (including
the null injection control, which exercises the full injection code path
with a zero vector). The hybrid architecture poses no challenge: the
