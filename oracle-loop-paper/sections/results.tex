...
All three models show similar behavioral patterns: approximately 45\%
correct on factual questions, 34\% hedged on confabulation-inducing
questions, and 17\% confidently confabulating. The confabulation rate
is consistent across models, suggesting the prompts reliably elicit
the target behavior.
...
($R^2 = 0.006$), which is problematic on the other models. The MP and
...
we cannot attribute this entirely to generation-phase cognitive differences.
...
discrimination for all five features (positive = confabulated), while
Llama reverses all signs. This is consistent with our prior finding that
geometric signatures are architecture-specific in direction while the
\emph{magnitude} of discrimination is universal. For a safety monitor,
this means per-model calibration is required---a threshold tuned on Qwen
will not transfer directly to Llama.
...
mp\_norm\_per\_token  & 0.129 & 0.102 & 0.006 \\
\bottomrule
\end{tabular}
...
For practical deployment, this means the monitor should be calibrated
per-model. This is not a limitation of the approach---it is a property
of the signal. Different architectures represent confabulation differently
in their key caches, even though all architectures \emph{do} represent it.
