...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Proof-of-concept activation steering} with a null
    injection control verifying character-level identical output on all
    100 trials---demonstrating that detection-gated correction works
    ($p = 0.031$) while blanket correction does not ($p = 0.36$)
    (\cref{sec:steering}).
  \item \textbf{A Cache Integrity Monitor} that detects unauthorized
    KV-cache modifications with 0\% false positives and 100\% true
    positives, released concurrently to ensure the defense is available
    alongside the attack methodology (\cref{sec:cache_defense}).
\end{enumerate}
